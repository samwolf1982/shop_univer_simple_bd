...
$price=rand(500,1000)+rand(1, 10) / 10;
  $sql_drop_db = "INSERT INTO `fotograf`( `surname`, `name`, `patronymic`, `price`) VALUES ('{$surnames_arr[$i]}','{$names_arr[$i]}','{$patronymik_arr[$i]}',{$price})";


mysql_query($sql_drop_db) or die(mysql_error().__LINE__);

echo nl2br("Фотограф {$surnames_arr[$i]} {$names_arr[$i]} {$patronymik_arr[$i]} добавлен\n");
}
